...
public class AnnotationProcessor {
    ...
    private void populateSchemaModel(final org.eclipse.microprofile.openapi.models.media.Schema schema,
            final Class<?> implementation) {
        schema.items(new SchemaImpl()); // todo
    }

    public void processClass(final OpenAPI api, final AnnotatedElement annotatedType,
        ...
        methods.filter(
                ...
                .forEach(m -> {
                    ...
                    if (m.isAnnotationPresent(GET.class)) {
                        getPathItem(api, completePath).setGET(buildOperation(api.getComponents(), m));
                    } else if (m.isAnnotationPresent(PUT.class)) {
                        getPathItem(api, completePath).setPUT(buildOperation(api.getComponents(), m));
                    } else if (m.isAnnotationPresent(POST.class)) {
                        getPathItem(api, completePath).setPOST(buildOperation(api.getComponents(), m));
                    } else if (m.isAnnotationPresent(HEAD.class)) {
                        getPathItem(api, completePath).setHEAD(buildOperation(api.getComponents(), m));
                    } else if (m.isAnnotationPresent(OPTIONS.class)) {
                        getPathItem(api, completePath).setOPTIONS(buildOperation(api.getComponents(), m));
                    } else if (m.isAnnotationPresent(PATCH.class)) {
                        getPathItem(api, completePath).setPATCH(buildOperation(api.getComponents(), m));
                    } else if (m.isAnnotationPresent(DELETE.class)) {
                        getPathItem(api, completePath).setDELETE(buildOperation(api.getComponents(), m));
                    } else {
                        Stream.of(m.getAnnotations()).filter(it -> it.annotationType().isAnnotationPresent(HttpMethod.class))
                                .findFirst().ifPresent(http -> {
                                    ...
                                    if ("TRACE".equals(mtd)) {
                                        getPathItem(api, completePath).setTRACE(buildOperation(api.getComponents(), m));
                                    } // else: how to map it???
                                });
                    }
                });
    ...
    private org.eclipse.microprofile.openapi.models.media.Schema mapSchemaFromClass(final Type model) {
        ...
        if (Class.class.isInstance(model)) {
            populateSchemaModel(schema, Class.class.cast(model));
        }
        return schema;
    }

    private Operation buildOperation(
            final org.eclipse.microprofile.openapi.models.Components components, final AnnotatedMethodElement m) {
        ...
        of(m.getAnnotationsByType(Tag.class)).filter(s -> s.length > 0)
                .ifPresent(tags -> operation.tags(Stream.of(tags).map(Tag::name).collect(toList())));
        of(m.getAnnotationsByType(APIResponse.class)).filter(s -> s.length > 0).ifPresent(items -> {
            ...
            responses.putAll(Stream.of(items).collect(
                    toMap(it -> of(it.responseCode()).filter(c -> !c.isEmpty()).orElse("200"), it -> mapResponse(components, it), (a, b) -> b)));
        ...
        operation.parameters(Stream.of(m.getParameters())
                ...
                .map(it -> buildParameter(it).orElseGet(() -> new ParameterImpl().schema(mapSchemaFromClass(it.getType()))))
    ...
    private Optional<org.eclipse.microprofile.openapi.models.parameters.Parameter> buildParameter(
        ...
        return ofNullable(annotatedElement.getAnnotation(Parameter.class)).map(this::mapParameter);
    }
    ...
    private String buildPath(final Path path, final Path mtdPath) {
        return Stream.of(path, mtdPath).filter(Objects::nonNull).map(Path::value)
    ...
    private void processComponents(final OpenAPI api, final Components components) {
        ...
        if (components.securitySchemes().length > 0) {
            impl.securitySchemes(
                    Stream.of(components.securitySchemes()).collect(toMap(SecurityScheme::ref, this::mapSecurityScheme)));
        }
        ...
        if (components.headers().length > 0) {
            impl.headers(Stream.of(components.headers()).collect(toMap(
                    it -> of(it.name()).filter(n -> !n.isEmpty()).orElseGet(() -> mapRefToName(it.ref())), it -> mapHeader(impl, it))));
        }
        ...
        if (components.schemas().length > 0) {
            impl.schemas(Stream.of(components.schemas()).collect(toMap(
                    it -> of(it.name()).filter(n -> !n.isEmpty()).orElseGet(() -> mapRefToName(it.ref())), it -> mapSchema(impl, it))));
        }
        if (components.responses().length > 0) {
            final APIResponses responses = new APIResponsesImpl();
            responses.putAll(Stream.of(components.responses()).collect(
                    toMap(it -> of(it.responseCode()).filter(c -> !c.isEmpty()).orElse("200"), it -> mapResponse(impl, it), (a, b) -> b)));
    ...
    private org.eclipse.microprofile.openapi.models.security.OAuthFlows mapFlows(final OAuthFlows oAuthFlows) {
        ...
        if (isSet(authorizationCode)) {
            flows.authorizationCode(new OAuthFlowImpl().authorizationUrl(authorizationCode.authorizationUrl())
        ...
        if (isSet(clientCredentials)) {
            flows.clientCredentials(
                    new OAuthFlowImpl().tokenUrl(clientCredentials.tokenUrl()).scopes(createScopes(clientCredentials)));
        }
        final OAuthFlow password = oAuthFlows.password();
        if (isSet(password)) {
            flows.password(new OAuthFlowImpl().tokenUrl(password.tokenUrl()).scopes(createScopes(password)));
        }
        final OAuthFlow implicit = oAuthFlows.implicit();
        if (isSet(implicit)) {
            flows.implicit(new OAuthFlowImpl().tokenUrl(implicit.tokenUrl()).scopes(createScopes(implicit)));
        }
        return flows;
    }
    ...
    private void processCallbacks(final org.eclipse.microprofile.openapi.models.Components impl, final Callback[] callbacks) {
        if (callbacks.length > 0) {
            impl.setCallbacks(Stream.of(callbacks).collect(toMap(
                    it -> of(it.name()).filter(n -> !n.isEmpty()).orElseGet(() -> mapRefToName(it.ref())), it -> mapCallback(impl, it))));
        }
    }
        ...
        if (callback.operations().length > 0) {
            Stream.of(callback.operations()).forEach(co -> {
                ...
                if (co.responses().length > 0) {
                    final APIResponses responses = new APIResponsesImpl();
                    responses.putAll(Stream.of(co.responses()).collect(
                            toMap(it -> of(it.responseCode()).filter(c -> !c.isEmpty()).orElse("200"), it -> mapResponse(components, it))));
                    operation.responses(responses);
                }
                switch (co.method()) {
                case "GET":
    ...
    private org.eclipse.microprofile.openapi.models.responses.APIResponse mapResponse(
        ...
        if (response.headers().length > 0) {
            impl.headers(Stream.of(response.headers()).collect(toMap(
                    it -> of(it.name()).filter(n -> !n.isEmpty()).orElseGet(() -> mapRefToName(it.ref())), it -> mapHeader(components, it))));
        }
        if (response.content().length > 0) {
            final ContentImpl content = new ContentImpl();
            content.putAll(Stream.of(response.content()).collect(toMap(Content::mediaType, it -> mapContent(components, it))));
    ...
    private org.eclipse.microprofile.openapi.models.parameters.RequestBody mapRequestBody(
        ...
        if (requestBody.content().length > 0) {
            final ContentImpl content = new ContentImpl();
            content.putAll(Stream.of(requestBody.content()).collect(toMap(Content::mediaType, it -> mapContent(components, it))));
            impl.content(content);
        }
        return impl;
    }
    ...
    private org.eclipse.microprofile.openapi.models.media.Schema mapSchema(
        ...
        if (schema.implementation() != Void.class) {
            populateSchemaModel(impl, schema.implementation());
        }
        of(schema.ref()).filter(v -> !v.isEmpty()).ifPresent(impl::ref);
